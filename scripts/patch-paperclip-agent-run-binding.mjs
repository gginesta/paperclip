...
const JWT_RUN_PATCH = `            ${JWT_RUN_MARKER}\n            const activeJwtRun = await db\n                .select({ id: heartbeatRuns.id })\n                .from(heartbeatRuns)\n                .where(and(eq(heartbeatRuns.id, claims.run_id), eq(heartbeatRuns.companyId, claims.company_id), eq(heartbeatRuns.agentId, claims.sub), eq(heartbeatRuns.status, "running")))\n                .then((rows) => rows[0] ?? null);\n            if (!activeJwtRun) {\n                next(forbidden("Signed agent JWT is not bound to an active run", { code: "agent_jwt_run_inactive" }));\n                return;\n            }\n`;
